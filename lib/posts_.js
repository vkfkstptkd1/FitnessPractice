...
export async function getRecommendPosts(userId) {
  let query = postsCollection.orderBy('createdAt', 'desc');

  if (userId) {
    query = query.where('user.displayName', '==', userId);
  }
...
export async function getPosts(followingid,id) {
    const followingid_= followingid
    let query = postsCollection.where('user.displayName','in',followingid_).orderBy('createdAt','desc').limit(PAGE_SIZE);
//if (followingid.length){
    //    console.log(followingid);
      //  query = query.where('user.displayName','in',followingid)
  //  }
    /*if(id){
...
export async function getOlderPosts(id, followingid){
    const cursorDoc = await postsCollection.doc(id).get();
    const followingid_= followingid
    let query = postsCollection.where('user.displayName','in',followingid_).orderBy('createdAt','desc').startAfter(cursorDoc).limit(PAGE_SIZE);
//if (followingid.length){
    //    console.log(followingid);
        //query = query.where('user.displayName','in',followingid)
  //  }
...
export async function getNewerPosts(id, followingid){
    const cursorDoc = await postsCollection.doc(id).get();
    const followingid_= followingid
    let query = postsCollection.where('user.displayName','in',followingid_).orderBy('createdAt','desc').endBefore(cursorDoc).limit(PAGE_SIZE);    //if (followingid.length){
        //    console.log(followingid);
        //    query = query.where('user.displayName','in',followingid)
      //  }
        /*if(id){
